...
const props = defineProps({
  ...
  desc: { type: String, required: false, default: "Desc Perusahaan" },
  status: { type: String, required: false, default: "aaaa" },
...
<template>
  <div>
    <div class="py-6 flex justify-center px-2">
      <div class="flex w-full bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-2xl">
        <div class="w-1/3 bg-cover" style="background-image: url('http://127.0.0.1:3000/img/photo/company-2.jpg')">
        </div>
        <div class="w-2/3 p-4">
          ...
          <div class="flex item-center mt-2">
            <div v-if="tersedia > 0" class="flex gap-1">
              ...
              <p class="text-sky-600">{{ props.status }}</p>
            </div>
          ...
          <div class="flex item-center justify-between mt-3">
            ...
            <div v-if="type == 'preview'">
              <button @click="doHapus(props.index)"
            ...
            <div v-else>
              <button @click="doPilih(props.id, props.title, props.tersedia, props.jmlTersedia)"
                class="px-3 py-2 bg-sky-600 text-white text-xs font-bold uppercase rounded"
                v-if="props.status === 'Tersedia'">
                Pilih
              </button>
              <!-- <button @click="doPilih(props.id, props.title, props.tersedia, props.jmlTersedia)"
                class="px-3 py-2 bg-sky-600 text-white text-xs font-bold uppercase rounded" v-if="tersedia > 0">
                Pilih
              </button> -->
              <button @click="doKetTidakTersedia()"
                class="px-3 py-2 bg-gray-400 text-gray-100 text-xs font-bold uppercase rounded" v-else>
                Pilih
              </button>
            </div>
          </div>
