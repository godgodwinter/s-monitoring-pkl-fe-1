...
const getPilihanTempatPKL = async () => {
  try {
    const response = await Api.get(
      `admin/pendaftaranpkl/getdatatempatpkl?cari=${inputCari.value}&tersedia=${inputTersedia.value ? inputTersedia.value.label : ""
      }`
    );
...
<template>
  ...
  <div class="px-3 py-3" v-else-if="statusPendaftaran == 'Proses Penempatan PKL'">
    <CardPemberitahuan title="Anda Sudah Mengisi Data Pengajuan Tempat PKL!"
      ket="Tunggu Proses Penempatan Tempat PKL oleh admin!" buttonTitle="Daftar Prakter Kerja Lapangan"
      button-link="MenuSiswaPendaftaranPklStep1" :isButtonActive="false" />

    <div class="grid grid-cols-1 xl:grid-cols-2">
      <div v-for="(item, index) in dataPengajuanPKL" :key="item.id">
        <CardCompany :title="item.tempatpkl.nama" :alamat="item.tempatpkl.alamat" :status="item.tempatpkl.status"
          tersedia="1" jmlTersedia="1/1" type="previewNoBtn"></CardCompany>
      </div>
      <!-- <CardCompany
          ...
          <div class="text-gray-700">
            <div class="grid md:grid-cols-2 text-sm">
              <div class="grid grid-cols-2">
                ...
                <div class="px-4 py-2">
                  <p class="text-gray-400 font-bold">
                    ...
                    <span class="text-gray-200 bg-gray-400 py-1 px-1 rounded-lg ml-2" v-if="dataTempatPkl.label"
                      @click="doBatal()">X</span>
                  </p>
                </div>
              </div>
              <div class="grid grid-cols-2">
              ...
              <div class="grid grid-cols-2">
                ...
                <div class="px-4 py-2">
                  <span class="bg-orange-400 py-2 px-2 rounded-lg text-white font-bold text-sm">Belum Daftar</span>
                </div>
              </div>
            </div>
            <!-- {{ stateTempatPKlSelected }} -->
            <div class="grid grid-cols-1 xl:grid-cols-2">
              <div v-for="(item, index) in stateTempatPKlSelected" :key="item.id">
                <CardCompany :index="index" :title="item.title" :alamat="item.alamat" :tersedia="item.tersedia"
                  :status="item.status" :jmlTersedia="item.jmlTersedia" type="preview"></CardCompany>
              </div>
            ...
            <div class="flex justify-end py-4 px-4">
              <div class="flex space-x-4 mb-6 text-sm font-medium">
                <div class="flex-auto flex space-x-4">
                  <button class="h-10 px-6 font-semibold rounded-md bg-emerald-500 text-white" type="submit"
                    v-if="stateTempatPKlSelected.length >= 2" @click="doSubmit()">
                    Simpan dan Lanjutkan Proses
                  </button>

                  <button class="h-10 px-6 font-semibold rounded-md bg-gray-400 text-gray-200" type="submit" v-else>
                    Simpan dan Lanjutkan Proses
                  </button>
                  <!-- <button
      ...
      <div class="bg-white p-3 shadow-sm rounded-sm">
        ...
        <div class="bg-white p-3 shadow-sm rounded-sm">
          ...
          <div class="flex flex-col md:flex-row justify-center gap-2">
            <v-select class="py-2 px-3 w-72 mx-auto md:mx-0" :options="statusPerusahaan" v-model="inputTersedia"
            ...
            <div>
              <div class="w-full md:w-72 pt-4 md:pt-2 mx-auto md:mx-0">
                <div class="relative">
                  ...
                  <input v-model="inputCari" @keyup="doCari()" type="search"
          ...
          <div class="text-gray-700">
            <div class="grid sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-2 text-sm">
              ...
              <div v-for="(item, index) in tempatPKL" :key="item.id">
                <CardCompany :id="item.id" :title="item.nama" :desc="item.desc" :alamat="item.alamat"
                  :status="item.status === 'Tersedia' ? 'Tersedia' : 'Tidak Tersedia'" :tersedia="item.tersedia"
                  :jmlTersedia="item.kuota">
                </CardCompany>
                <!-- <CardCompany :id="item.id" :title="item.nama" :desc="item.desc" :alamat="item.alamat"
                  :status="item.status === 'Tersedia' ? 'Tersedia' : 'Tidak Tersedia'" :tersedia="item.tersedia"
